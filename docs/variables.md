...
| `enable_key_vault_private_endpoint` | bool | `false` | Creates the Key Vault private endpoint when `true`. Leave `false` unless the Terraform runner can resolve and reach the vault's private IP path; otherwise `azurerm_key_vault` refresh can fail on `GetCertificateContacts` timeouts |
